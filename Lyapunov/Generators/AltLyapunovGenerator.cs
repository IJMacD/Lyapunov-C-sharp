...
namespace Lyapunov
{
    class AltLyapunovGenerator : Generator
    {
        ...
        {/*
            ...
            {*/
                ...
                double[,] sum_of_log_of_derived = new double[_PicWidth, _PicHeight];
                double[,] _x = new double[_PicWidth, _PicHeight];
                    ...
                    for (currZ = 0; currZ < _PicDepth; currZ++)
                    {
                        ...
                        for (currX = 0; currX < _PicWidth; currX++)
                        {
                            for (int currY = 0; currY < _PicHeight; currY++)
                            {
                                ...
                                sum_of_log_of_derived[currX, currY] += log_of_derived;
                                if (double.IsInfinity(derived))
                                    break;
                                ...
                                double value = sum_of_log_of_derived[currX, currY] / (_Iterations + _Pattern.Length);
                                ...
                                lock (padlock)
                                {
                                    _Images[currZ].SetPixel(currX, (_PicHeight - currY) - 1, pix);
                                }
                            }
                            if (_Worker.CancellationPending)
                                    return;
                        }
                        float frac = (float)n / (float)_Iterations;
                        _Worker.ReportProgress((int)(frac*100.0));
                        if (allblue)
                            _Worker.ReportProgress(-2, currZ);
                        else
                            _Worker.ReportProgress(-1, currZ);
                    }
                }
                _Completed = true;
                /*
